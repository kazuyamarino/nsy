...
class NSY_CSRF
{
     ...
     */
    protected static $do_origin_check = false;
     ...
     */
    public static function check($key, $origin, $throwException = false, $time_span = null, $multiple = false)
    {

        if (!isset($_SESSION['csrf_' . $key])) {
            return self::returnOrException($throwException, 'Missing CSRF session token.');
        }

        if (!isset($origin[$key])) {
            return self::returnOrException($throwException, 'Missing CSRF form token.');
        }
        ...
        // Origin checks
        if (self::$do_origin_check && hash('SHA256', $_SERVER['REMOTE_ADDR'] . $_SERVER['HTTP_USER_AGENT'])
            != substr(base64_decode($hash), 10, 40)) {
            return self::returnOrException($throwException, 'Form origin does not match token origin.');
        }

        // Check if session token matches form token
        if ($origin[$key] != $hash) {
            return self::returnOrException($throwException, 'Invalid CSRF token.');
        }
        ...
        if ($time_span !== null &&
            ...
            return self::returnOrException($throwException, 'CSRF token has expired.');
     ...
     * @throws \Exception
     */
    private static function returnOrException(bool $throwException, string $exceptionString){
        if ($throwException) {
